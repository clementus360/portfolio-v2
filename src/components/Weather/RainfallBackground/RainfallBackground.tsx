...
type RainfallBackgroundProps = {
    enabled?: boolean;
    /** Roughly 1 (drizzle) → 3 (thunderstorm). Drives drop count, length and speed. */
    intensity?: number;
...
const RainfallBackground: React.FC<RainfallBackgroundProps> = ({
    ...
    const drops = useMemo(() => {
        ...
        return Array.from({ length: count }, (_, i) => {
            const duration = clamp(baseDuration + Math.random() * 0.4 - 0.1, 0.4, 1.5);
            return {
                id: i,
    ...
    // time slightly, but it always stays in a calm, readable range.
    const angle = clamp(windKph * 0.4, 0, 24);
